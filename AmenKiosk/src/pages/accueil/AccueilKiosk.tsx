// src/pages/kiosk/AccueilKiosk.tsx
...
// Define local interfaces (assuming transaction structure is similar to AuthContext.tsx)
interface Account {
  _id: string;
  numéroCompte: string;
  solde: number;
...
const AccueilKiosk: React.FC = () => {
  ...
  // Extract accounts and cards from profile data
  ...
  // Combine transactions from all accounts' "historique" field instead of fetching from an API
  const transactions: Transaction[] = useMemo(() => {
    if (!profile) return [];
    const allTransactions = profile.comptes.reduce((acc: Transaction[], compte) => {
      return compte.historique ? [...acc, ...compte.historique] : acc;
    }, []);
  ...
  // Calculate overall total balance from accounts
  const totalBalance = useMemo(
    () => accounts.reduce((sum, account) => sum + account.solde, 0),
    [accounts]
  );

  // Prepare chart data by grouping transactions by month
  const chartData = useMemo(() => {
    const groupedData = transactions.reduce(
      (acc, transaction) => {
        ...
        if (!acc[key]) {
          acc[key] = { name: month, fullDate: date, income: 0, expenses: 0, savings: 0 };
        }
  ...
  const handlePrintStatement = async () => {
    try {
      ...
      // Calculate total credits, debits, and net balance from transactions
      ...
      const defaultBankBranding = {
        name: "Amen Bank",
        logo: "amen_logo.png", // Ensure path is correct
        ...
        <div className="accueil-kiosk-container">
          <div className="accueil-kiosk-content">
            <div className="accueil-kiosk-header">
              <IonImg className="Logo" src="amen_logo.png" alt="Amen Bank Logo" />
